...
if "$BTCPAYSERVER" && [[ -z "$BTCPAY_HOST" ]]; then
...
if "$BTCPAYSERVER" && [[ -z "$BTCPAY_LN" ]]; then
...
"
sudo -u standup git clone https://github.com/btcpayserver/btcpayserver-docker ~standup/downloads/btcpayserver-docker
cd ~standup/downloads/btcpayserver-docker
...
"
ln -s /home/standup/.bitcoin /var/lib/docker/volumes/generated_bitcoin_datadir/_data

# start btcpay server
echo "
$MESSAGE_PREFIX starting btcpayserver
"
. ./btcpay-up.sh
